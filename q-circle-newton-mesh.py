...
if __name__ == "__main__":
    ...
    # time.sleep(1)
    data = exchange.import_json("curved_beam_lineload_2.json") #  pinched_shell_kninsertion_changedeg.json pinched_shell.json rectangle_cantilever square square_kninsertion generic_shell_kninsertion foursided_curved_kninsertion foursided_curved_kninsertion2  rectangle_kninsertion
        # visualization(data)
    ...
    while i_main <= max_order_elem:
        ...
        while j_main <= max_number_elem:
            ...
            with open(f'q_circle_jacobian_{i_main}.dat', 'a') as jac:
                for k in range(2):
                    for i in range(dim):
                        for j in range(dim):
                            np.savetxt(jac, jacobian_all[k, 0, i, j])
                            jac.write("\n")
                    jac.write("\n\n\n\n")
            ...
            bc = gsmsml.global_boundary_condition(lobatto_pw, bc_h_bott, bc_h_top,\
                                    bc_v_left, bc_v_right, element_boundaries_u,\
                                    element_boundaries_v)
            mid_point =  int((node_global_d - 1)/2 + 1)
            loaded_dof = 5 * mid_point - 3
            global_total_load = np.zeros(total_dof)
            global_total_load[loaded_dof] = point_load
            ...
            output_array = np.array([[i_main, number_element_u,\
                            number_element_v,\
